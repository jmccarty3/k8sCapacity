...
function write_data {
  curl -s -i -XPOST $INFLUXDB_ADDRESS/write?db=$db --data-binary "$BATCH_DATA"
}
...
if [ -f $BEARER_TOKEN ]; then
  ...
  CURL_CMD="curl -s --header \"Authorization: Bearer $BEARER_TOKEN\" --insecure https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT"
elif [ -n "$KUBERNETES_MASTER" ]; then
  CURL_CMD="curl -s --insecure http://$KUBERNETES_MASTER"
else
